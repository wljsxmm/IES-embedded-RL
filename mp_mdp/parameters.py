# 每小时的负荷
hourly_demand = [2130, 2108, 2196, 2214, 2112, 2140, 2262, 2400, 2350, 2182, 2098, 2038, 1915, 1860, 1800, 1782, 1702,
                 1696, 1694, 1716, 1770, 1792, 1864, 1946]

# 将负荷降低50%
hourly_demand = [i * 0.58 for i in hourly_demand]
# print(max(hourly_demand), min(hourly_demand)) # 1392.0 982.52
...
# 生成每小时的热负荷都为900
hourly_heat_demand = [600 for i in range(24)]


hourly_wind_power_low = [200 for i in range(24)]
hourly_wind_power_middle = [250 for i in range(24)]
hourly_wind_power_high = [350 for i in range(24)]


hourly_wind_power_available_low = [255, 233, 294, 286, 202, 190, 281, 217, 223, 235, 255, 260, 268, 270, 269, 250, 241, 228, 268, 278, 288, 200, 280, 262]

hourly_wind_power_available = [255, 233, 194, 186, 202, 290, 281, 217, 223, 235, 255, 260, 368, 270, 269, 250, 341, 358, 368, 278, 288, 300, 280, 262]

hourly_wind_power_available_high = [255, 233, 294, 186, 202, 290, 381, 317, 323, 235, 355, 360, 368, 370, 369, 350, 341, 358, 368, 378, 388, 300, 380, 362]
...
hst_status = [[0,200], [200,400],[400,600],[600,800],[800,1000]]

import itertools


# Generate all combinations of statuses for 4 hsts
combined_status = list(itertools.product(hst_status, repeat=2))
...
#            "cv2": 0.15, "cm": 0.75, "Hmax": 450, "ramp_up": 70, "ramp_down": 70},
#     "G4": {"a": 0.000076, "b": 0.2716, "c": 18.822, "Pmin": 150, "Pmax": 300, "heat_ratio": 0.3, "cv1": 0.15,
#            "cv2": 0.15, "cm": 0.75, "Hmax": 400, "ramp_up": 80, "ramp_down": 80},
#     "G5": {"a": 0.000076, "b": 0.2716, "c": 18.822, "Pmin": 150, "Pmax": 300, "heat_ratio": 0.3, "cv1": 0.15,
#            "cv2": 0.15, "cm": 0.75, "Hmax": 400, "ramp_up": 80, "ramp_down": 80},
#     "G6": {"a": 0.000076, "b": 0.2716, "c": 18.822, "Pmin": 150, "Pmax": 300, "heat_ratio": 0.3, "cv1": 0.15,
#            "cv2": 0.15, "cm": 0.75, "Hmax": 400, "ramp_up": 80, "ramp_down": 80}
# }

generators_chp = {
    "G1": {"a": 0.000076, "b": 0.2716, "c": 18.822, "Pmin": 450, "Pmax": 1100, "heat_ratio": 0.3, "cv1": 0.15,
           "cv2": 0.15, "cm": 0.75, "Hmax": 750, "ramp_up": 240, "ramp_down": 240}
}
...
generators_condensing = {
    "G2": {"a": 0.001171, "b": 0.2705, "c": 11.537, "Pmin": 150, "Pmax": 400, "ramp_up": 100, "ramp_down": 100},
}
# 区域a和区域b的储热设备参数
# thermal_storage = {
...
thermal_storage = {
    "a": {"capacity": 1000, "initial_heat": 500, "charge_capacity": 100, "discharge_capacity": 100}
}
