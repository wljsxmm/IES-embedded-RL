...
def economic_dispatch_test(hourly_demand, hourly_heat_demand, wind_scenarios, action):
    """
    简单系统用于测试 末状态的优化对风电消纳的作用
    """
    model = gp.Model("EconomicDispatchTest")
    ...
    for scenario_idx in range(len(wind_scenarios)):
        ...
        for hour in range(len(hourly_demand)):
            for name in generators_chp.keys():
                ...
                scenario_cost += power_vars_chp[hour, name, scenario_idx] * power_vars_chp[hour, name, scenario_idx] * \
                                 generators_chp[name]["a"] + \
                                 power_vars_chp[hour, name, scenario_idx] * generators_chp[name]["b"] + \
                                 generators_chp[name]["c"]
    ...
    for scenario_idx, wind_scenario in enumerate(wind_scenarios):
        for hour in range(len(hourly_demand)):
            ...
            # 热力平衡
            model.addConstr(
                gp.quicksum(heat_vars_chp[hour, name, scenario_idx] for name in generators_chp.keys())
                + gp.quicksum(heat_discharge_vars[hour, name, scenario_idx] for name in thermal_storage.keys())
                == hourly_heat_demand[hour] + gp.quicksum(
                    heat_charge_vars[hour, name, scenario_idx] for name in thermal_storage.keys()),
            ...
            for name in generators_chp.keys():
                ...
                model.addConstr(
                    generators_chp[name]["cm"] * power_vars_chp[hour, name, scenario_idx] == heat_vars_chp[
                        hour, name, scenario_idx])
...
def economic_dispatch_test_optimal(hourly_demand, hourly_heat_demand, wind_scenarios):
    """
    简单系统用于测试 末状态的优化对风电消纳的作用
    """
    model = gp.Model("EconomicDispatchTest")
...
def economic_dispatch_test_base(hourly_demand, hourly_heat_demand, wind_scenarios):
    """
    简单系统用于测试 末状态的优化对风电消纳的作用
    """
    model = gp.Model("EconomicDispatchTest")
    ...
    for scenario_idx, wind_scenario in enumerate(wind_scenarios):
        ...
        for name in thermal_storage.keys():
            if scenario_idx == 0:
                ...
                # Add constraints for scenario 2
                model.addConstr(
                    storage_heat_vars[0, name, scenario_idx] == storage_heat_vars[
                        len(hourly_demand), name, scenario_idx - 1],
                    name=f"StorageEnergy_initial_scenario{scenario_idx}_HST{name}")
                model.addRange(storage_heat_vars[len(hourly_demand), name, scenario_idx], initial_storage_heat[2][0],
                               initial_storage_heat[2][1],
                               name=f"StorageEnergy_final_scenario{scenario_idx}_HST{name}")
            elif scenario_idx == 3:
                # Add constraints for scenario 3
                model.addConstr(
                    storage_heat_vars[0, name, scenario_idx] == storage_heat_vars[
                        len(hourly_demand), name, scenario_idx - 1],
                    name=f"StorageEnergy_initial_scenario{scenario_idx}_HST{name}")
                model.addRange(storage_heat_vars[len(hourly_demand), name, scenario_idx], initial_storage_heat[3][0],
                               initial_storage_heat[3][1],
                               name=f"StorageEnergy_final_scenario{scenario_idx}_HST{name}")
            elif scenario_idx == 4:
                model.addConstr(
                    storage_heat_vars[0, name, scenario_idx] == storage_heat_vars[
                ...
                model.addConstr(
                    storage_heat_vars[len(hourly_demand), name, scenario_idx] == thermal_storage[name]["initial_heat"],
                    name=f"StorageEnergy_final_scenario{scenario_idx}_HST{name}")
            # Add constraints for scenario 4
            else:
